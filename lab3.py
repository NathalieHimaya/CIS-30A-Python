...
plt.bar(df['Title'], df['US_Gross'], color='skyblue')
plt.title("Marvel Movies - US Gross Revenue")
...
# Display the highest and lowest grossing movies
max_gross = df['US_Gross'].max()
min_gross = df['US_Gross'].min()

max_movie = df[df['US_Gross'] == max_gross]['Title'].values[0]
min_movie = df[df['US_Gross'] == min_gross]['Title'].values[0]

print(f"\nHighest Grossing Movie: {max_movie} (${max_gross})")
print(f"Lowest Grossing Movie: {min_movie} (${min_gross})")
